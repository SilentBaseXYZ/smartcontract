...
contract Arbitrage {
    ...
    function swap(address _address,uint256 _amountIn, bool reverse) external {
        ...
        if (!reverse) {
            ...
            path[0] = WETH_ADDRESS;
            path[1] = _address;
            amounts = uniswapRouter.swapExactTokensForTokens(_amountIn, 0, path, address(this), block.timestamp);
            amountOut = amounts[1];
            ...
            path[0] = _address;
            path[1] = WETH_ADDRESS;
            amounts_1 = sushiswapRouter.swapExactTokensForTokens(amountOut, 0, path, msg.sender, block.timestamp);
            amountOut_1 = amounts_1[1];
        } else {
            // Buy the token on Sushiswap with ETH
            path[0] = WETH_ADDRESS;
            path[1] = _address;
            amounts = sushiswapRouter.swapExactTokensForTokens(_amountIn, 0, path, address(this), block.timestamp);
            amountOut = amounts[1];

            // Sell the token on Uniswap with _address
            IWETH9(_address).approve(address(UNISWAP_ROUTER_ADDRESS) ,amountOut); 
            path[0] = _address;
            path[1] = WETH_ADDRESS;
            amounts_1 = uniswapRouter.swapExactTokensForTokens(amountOut, 0, path, msg.sender, block.timestamp);
            amountOut_1 = amounts_1[1];
        }
